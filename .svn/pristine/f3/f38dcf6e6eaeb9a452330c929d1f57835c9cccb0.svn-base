  ...
  def initializeParticles(self):
    ...
    for i in range(self.numParticles): 
    	...
    	for j in range(self.numGhosts): 
    		pos.append(random.choice(self.legalPositions))
    	self.particles.append(tuple(pos))
    ...
    for particle in self.particles:
    	...
    	for i in range(self.numGhosts): 
    		if noisyDistances[i] == None: 
    			list(particle)[i] = self.getJailPosition(i)
    		else: 
    			distance = util.manhattanDistance(list(particle)[i], pacmanPosition) 
    			weight = weight * emissionModels[i][distance] 
    	allPossible[tuple(list(particle))] += weight
