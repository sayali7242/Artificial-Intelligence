...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    for livingGhostPosition in livingGhostPositionDistributions: 
    	ghostDistance = self.distancer.getDistance(pacmanPosition, livingGhostPosition.argMax())
    	if minDistance > ghostDistance: 
    		minDistance = ghostDistance 
    		oldPos = livingGhostPosition.argMax()
    		
    newMinDistance = float('inf')
    for action in legal: 
			newPos = Actions.getSuccessor(pacmanPosition, action) 
			newDistance = self.distancer.getDistance(oldPos, newPos)
			if newMinDistance > newDistance: 
